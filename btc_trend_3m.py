...
ENABLE_MONGODB = True  # Set to False to disable MongoDB features
...
def init_mongodb() -> bool:
    ...
    try:
        ...
        signals_col = mongo_db[MONGO_COLLECTIONS["signals"]]
        signals_col.create_index([("symbol", 1), ("timestamp", -1)])
        signals_col.create_index([("signal_id", 1)], unique=True)
        
        results_col = mongo_db[MONGO_COLLECTIONS["results"]]
        results_col.create_index([("signal_id", 1)])
        results_col.create_index([("symbol", 1), ("timestamp", -1)])
        
        market_col = mongo_db[MONGO_COLLECTIONS["market_data"]]
        market_col.create_index([("symbol", 1), ("timestamp", -1)])
...
def save_signal_to_mongo(signal_data: dict, trend_data: dict, rr_data: dict) -> bool:
    """Save trading signal to MongoDB."""
    if not mongo_db or not ENABLE_MONGODB:
        return False
    ...
    try:
        signal_doc = {
            ...
            "take_profit_1": rr_data["tp1"],
            "take_profit_2": rr_data["tp2"],
            "take_profit_3": rr_data["tp3"],
            "risk_reward_ratio": rr_data["rr"],
            "confidence": rr_data["confidence"],
            "strategy": rr_data["strategy"],
            "trend_3m": trend_data["trend_3m"],
            "trend_15m": trend_data["trend_15m"],
            "mtf_alignment": trend_data["mtf_alignment"],
            "volume_ratio": rr_data["volume_ratio"],
            "volatility_ratio": rr_data["volatility_ratio"],
            "atr": rr_data["atr"],
            "market_indicators": signal_data.get("market_indicators", []),
            "technical_data": {
                "ema50": trend_data["ema50"],
                "ema100": trend_data["ema100"],
                "rsi": trend_data["rsi"],
                "macd_line": trend_data["macd_line"],
                "macd_signal": trend_data["macd_signal"],
                "macd_hist": trend_data["macd_hist"],
                "volume": trend_data["volume"]
            },
            "status": "active",
            "outcome": None  # Will be updated when signal resolves
        }
        
        collection = mongo_db[MONGO_COLLECTIONS["signals"]]
        collection.insert_one(signal_doc)
        print(f"💾 Signal saved to MongoDB: {signal_data['signal_id']}")
        return True
        
    except Exception as e:
        print(f"❌ Error saving signal to MongoDB: {e}")
        return False

def save_market_analysis_to_mongo(symbol: str, analysis_data: dict) -> bool:
    """Save market analysis data to MongoDB."""
    if not mongo_db or not ENABLE_MONGODB:
        return False
    ...
    try:
        analysis_doc = {
            ...
            "trend_score_3m": analysis_data["trend_score_3m"],
            "trend_score_15m": analysis_data["trend_score_15m"],
            "price_data": {
                "close": analysis_data["close"],
                "ema50": analysis_data["ema50"],
                "ema100": analysis_data["ema100"]
            },
            "indicators": {
                "rsi": analysis_data["rsi"],
                "macd_line": analysis_data["macd_line"],
                "macd_signal": analysis_data["macd_signal"],
                "macd_hist": analysis_data["macd_hist"],
                "volume_ratio": analysis_data.get("volume_ratio", 1.0),
                "volatility_ratio": analysis_data.get("volatility_ratio", 1.0)
            },
            "signals_detected": analysis_data.get("signals_count", 0),
            "market_regime": analysis_data.get("market_regime", "unknown")
        }
        
        collection = mongo_db[MONGO_COLLECTIONS["market_data"]]
        collection.insert_one(analysis_doc)
        return True
        
    except Exception as e:
        print(f"❌ Error saving market analysis to MongoDB: {e}")
        return False

def get_signal_performance_stats(symbol: str = None, days: int = 7) -> dict:
    """Get signal performance statistics from MongoDB."""
    if not mongo_db or not ENABLE_MONGODB:
        return {"total_signals": 0, "win_rate": 0.0, "avg_rr": 0.0, "long_win_rate": 0.0, "short_win_rate": 0.0}
    ...
    try:
        ...
        # Build query filter
        query_filter = {
            "timestamp": {"$gte": datetime.now() - pd.Timedelta(days=days)},
            "outcome": {"$ne": None}
        }
        if symbol:
        ...
        signals = list(collection.find(query_filter))
        ...
        long_signals = [s for s in signals if s["side"] == "bullish"]
        short_signals = [s for s in signals if s["side"] == "bearish"]
        
        long_wins = len([s for s in long_signals if s["outcome"] == "win"])
        short_wins = len([s for s in short_signals if s["outcome"] == "win"])
        
        long_win_rate = (long_wins / len(long_signals)) * 100 if long_signals else 0.0
        short_win_rate = (short_wins / len(short_signals)) * 100 if short_signals else 0.0
        
        # Calculate average R:R
        rr_values = [s["risk_reward_ratio"] for s in signals if s.get("risk_reward_ratio")]
        avg_rr = sum(rr_values) / len(rr_values) if rr_values else 0.0
        
        # Calculate average R:R by side
        long_rr_values = [s["risk_reward_ratio"] for s in long_signals if s.get("risk_reward_ratio")]
        short_rr_values = [s["risk_reward_ratio"] for s in short_signals if s.get("risk_reward_ratio")]
        
        long_avg_rr = sum(long_rr_values) / len(long_rr_values) if long_rr_values else 0.0
        short_avg_rr = sum(short_rr_values) / len(short_rr_values) if short_rr_values else 0.0
        ...
        if signals:
            ...
            if first_run or signal_id != last_signal_ids.get(symbol):
                ...
                if mtf_ok and rr_ok and enhanced_confidence_ok:
                    ...
                    if market_structure.get("success", False):
                        market_indicators.append(f"🏗️ {market_structure['structure_rating']}")
                    
                    # Add volume profile info
                    if volume_profile.get("success", False) and volume_profile.get("price_near_poc"):
                        market_indicators.append("📊 Near POC")
                    
                    # Add confluence info
                    if confluence_analysis["confluence_score"] >= 2:
                        market_indicators.append(f"🎯 Confluence: {confluence_analysis['confluence_score']:.1f}")
                    
                    market_text = " | ".join(market_indicators) if market_indicators else "📊 Normal Conditions"
                    
                    # Enhanced signal message with clear Entry/SL/TP
                    msg_parts = [
                        f"<b>🚨 {symbol} {latest['type']} Signal</b>",
                        f"📊 Direction: <b>{side.upper()}</b> | Confidence: <b>{rr_data['confidence']:.0%}</b>",
                        f"⏰ {signal_time}",
                        f"",
                        f"📈 <b>TRADE SETUP:</b>",
                        f"💰 Entry: <b>{fmt(entry_price)}</b>",
                        f"🔴 Stop Loss: <b>{fmt(sl_price)}</b> ({risk_percent:.1f}%)",
                        tp_text,
                        f"",
                        f"📊 Risk:Reward = <b>1:{fmt(rr_data['rr'], 2)}</b>",
                        f"🎯 Strategy: {rr_data['strategy']}",
                        f"",
                        f"📈 <b>MARKET ANALYSIS:</b>",
                        f"🕐 3m Trend: {result3['label']} | 15m: {result15['label']}",
                        f"📋 Current: {result3['last_close']:.2f} | EMA50: {result3['ema50']:.2f} | RSI: {result3['rsi']:.1f}",
                        f"🔍 {market_text}"
                    ]
                    ...
                    print(f"✅ {symbol}: Sending ENHANCED signal (R:R={rr_data['rr']:.2f}, Final Confidence={rr_data['confidence']:.2f})")
                    ...
                    if ENABLE_MONGODB:
                        ...
                        trend_mongo_data = {
                            ...
                            "mtf_alignment": tag_15m,
                            "ema50": result3['ema50'],
                            "ema100": result3['ema100'],
                            "rsi": result3['rsi'],
                            "macd_line": result3['macd_line'],
                            "macd_signal": result3['macd_signal'],
                            "macd_hist": result3['macd_hist'],
                            "volume": result3['volume']
                        }
                        
                        save_signal_to_mongo(signal_mongo_data, trend_mongo_data, rr_data)
                        
                        # Save market analysis
                        market_analysis_data = {
                            "analysis_time": result3['last_time'],
                            "trend_score_3m": result3,
                            "trend_score_15m": result15,
                            "close": result3['last_close'],
                            "ema50": result3['ema50'],
                            "ema100": result3['ema100'],
                            "rsi": result3['rsi'],
                            "macd_line": result3['macd_line'],
                            "macd_signal": result3['macd_signal'],
                            "macd_hist": result3['macd_hist'],
                            "volume_ratio": rr_data.get('volume_ratio', 1.0),
                            "volatility_ratio": rr_data.get('volatility_ratio', 1.0),
                            "signals_count": len(signals),
                            "market_regime": result3['label']
                        }
                        
                        save_market_analysis_to_mongo(symbol, market_analysis_data)
            ...
            for symbol in SYMBOLS:
                ...
                if success:
                    ...
                    try:
                        ...
                        # Send strong recommendation notifications
                        if recommendation['action'] in ['STRONG LONG', 'STRONG SHORT']:
                            ...
                            # Only send if it's a new strong recommendation
                            if last_rec != current_rec_id:
                                ...
                                for reason in recommendation['reasons'][:3]:  # Top 3 reasons
                                    rec_msg_parts.append(f"   • {reason}")
                                
                                rec_msg_parts.extend([
                                    f"",
                                    f"⚡ Market Conditions:",
                                    f"   📊 Structure: {'✅' if recommendation['structure_supports'] else '❌'} | Volume: {'✅' if recommendation['volume_supports'] else '❌'} | MTF: {'✅' if recommendation['timeframe_alignment'] else '❌'}",
